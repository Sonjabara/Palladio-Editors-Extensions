...
import org.palladiosimulator.pcm.dataspec.seff.AcquireDataAction;
import org.palladiosimulator.pcm.repository.BasicComponent;
import org.palladiosimulator.pcm.repository.Repository;
import org.palladiosimulator.pcm.seff.ServiceEffectSpecification;

public class AddAcquireDataAction implements IExternalJavaAction {
	
	public static final Shell SHELL = PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell();

	@Override
	public void execute(Collection<? extends EObject> selections, Map<String, Object> parameters) {
		AcquireDataAction acquireDataAction = (AcquireDataAction) parameters.get("instance");
		DataPort dataPort = getDataPort(acquireDataAction);
		acquireDataAction.setDataPort(dataPort);
	}
	
	private DataPort getDataPort(AcquireDataAction acquireDataAction) {	
		...
		PalladioSelectEObjectDialog dialog = new PalladioSelectEObjectDialog(SHELL, filter, additionalChildReferences,
				acquireDataAction.eResource().getResourceSet());	
		dialog.setProvidedService(DataPort.class);
		
		ServiceEffectSpecification seff = (ServiceEffectSpecification) acquireDataAction.getResourceDemandingBehaviour_AbstractAction();
	 ...
	 * 
	 * @param basicComponent  The container of the SEFF, which contains the acquireDataAction
	 * @return a filtered stream consisting of DataPorts, that do not have an inbound datatype
	 * 		   or an empty stream, if the basicComponent does not contain DataPorts without inbound datatypes
	 */
	private Stream<DataPort> getDataPortsToRemove(BasicComponent basicComponent) {
		return basicComponent.getProvidedRoles_InterfaceProvidingEntity().stream()
				...
				.filter(dataPort -> dataPort.getPortSpecification().getInboundDataType() == null);
	}
